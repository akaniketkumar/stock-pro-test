...
export default async function handler(req, res) {
  ...
  async function fetchChart(rangeParam, intervalParam) {
    const url = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(yahooSymbol)}?range=${rangeParam}&interval=${intervalParam}`
  ...
  try {
    // Some range+interval combinations Yahoo is picky about (e.g. very long
    // ranges with a coarse interval). If the requested combo comes back
    // empty, retry once with a safer, more universally-accepted interval
    // for that same range instead of silently falling back to fake data.
    let result = await fetchChart(safeRange, interval)
    let usedInterval = interval
    if (!result) {
      const fallbackInterval = interval === '1mo' ? '1wk' : interval === '1wk' ? '1d' : interval === '5m' ? '15m' : '1d'
      result = await fetchChart(safeRange, fallbackInterval)
      usedInterval = fallbackInterval
    }
    ...
    const candles = timestamps
      .map((t, i) => {
        ...
        return {
          date: usedInterval.endsWith('m') ? iso : iso.slice(0, 10),
          time: t,
          open,
          high,
          low,
    ...
    const cacheSeconds = usedInterval.endsWith('m') ? 60 : 900
    res.setHeader('Cache-Control', `s-maxage=${cacheSeconds}, stale-while-revalidate=${cacheSeconds * 4}`)

    return res.status(200).json({ success: true, symbol: cleanSymbol, interval: usedInterval, count: candles.length, candles })
  } catch (error) {
