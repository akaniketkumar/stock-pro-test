...
// 👇 YAHAN TUM APNI WEBSITE KA NAYA NAAM RAKH SAKTE HO 👇
const BRAND_NAME_1 = "Stock"
const BRAND_NAME_2 = "Pro" // Isko change karke apna brand name rakh lena
  ...
  return (
    <Link to="/" className="flex items-center gap-2 transition-transform hover:scale-105">
      ...
      <span className="text-lg font-extrabold tracking-tight text-white">
        {BRAND_NAME_1}<span className="text-sky-400">{BRAND_NAME_2}</span>
      </span>
    </Link>
  )
}
      ...
      <div className="mx-auto max-w-7xl px-4">
        <div className="flex h-16 items-center gap-4">
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
                    isActive ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-400 hover:bg-slate-800/60 hover:text-slate-200'
                  }`
                }
              >
                {link.label}
          ...
          <div className="ml-auto flex items-center gap-2 md:ml-0">
            <Link
              to="/#watchlist"
              aria-label={`Watchlist (${watchlist?.length || 0} saved)`}
            ...
            >
              ...
              {watchlist?.length > 0 && (
                <span className="absolute -right-1.5 -top-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-amber-400 px-1 font-mono text-[9px] font-bold text-terminal-950 shadow-sm">
                  {watchlist.length}
                </span>
              )}
            </Link>
            {user ? (
              <div className="flex items-center gap-2">
                ...
                <button type="button" onClick={logout} className="btn-ghost hidden px-3 py-1.5 sm:inline-flex hover:text-rose-400 transition-colors">
                  Logout
                </button>
              </div>
            ) : (
            ...
            <button
              ...
              className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-700 text-slate-300 lg:hidden hover:bg-slate-800 transition-colors"
            >
        ...
        {menuOpen && (
          <nav className="flex flex-col gap-1 border-t border-slate-800 py-3 lg:hidden animate-fade-in">
            {NAV_LINKS.map((link) => (
              <NavLink
                key={link.to}
                to={link.to}
                ...
                className={({ isActive }) =>
                  `rounded-lg px-3 py-2 text-sm font-medium transition-colors ${isActive ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-slate-800/50'}`
                }
              >
                {link.label}
              </NavLink>
            ...
            {!user && (
              <div className="mt-2 flex gap-2">
                <Link to="/login" className="btn-ghost flex-1 justify-center">
                  Login
                </Link>
                <Link to="/signup" className="btn-primary flex-1 justify-center">
                  Sign up
                </Link>
              </div>
            )}
        ...
        <div className="border-t border-slate-800/60 bg-terminal-900/60">
          <div className="mx-auto flex max-w-7xl items-center overflow-hidden px-4">
            <span className="flex shrink-0 items-center gap-1 py-1.5 pr-3 text-[11px] font-bold uppercase tracking-wider text-sky-400">
              Market
              <Help text="Live index levels for major Indian benchmarks, updating with real-time API data." iconSize="h-3 w-3" />
            </span>
